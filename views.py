...
def coleta_dashboard_view(request):
    ...
    def sort_key(c):
        if c.data_agendada == hoje:
            return (0, c.ordem_visita)
        if c.data_agendada is None:
            return (1, c.ordem_visita)
        return (2, c.data_agendada, c.ordem_visita)

    for coleta in sorted(todas_as_coletas, key=sort_key):
...
def gerar_romaneio_view(request):
    ...
    for coleta in coletas_agendadas:
        ...
        if coleta.pedido_liberado:
            ...
            try:
                fornecedor_erp = SA2Fornecedor.objects.get(a2_cod=fornecedor_portal.codigo_externo.strip())
